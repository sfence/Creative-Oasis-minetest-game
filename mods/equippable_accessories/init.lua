...
-- Initialize table to avoid nil errors
if not player_acc_visuals[name] then
    player_acc_visuals[name] = {
        head = {},
        body = {},
        l_arm = {},
        r_arm = {},
...
function update_player_acc_visuals(player, accessory, addtype, accessorySlot, index)
    ...
    if addtype == 1 then
        ...
        player_acc_visuals[name] = player_acc_visuals[name] or {head={}, body={}, l_arm={}, r_arm={}, l_leg={}, r_leg={}}
        ...
        if minetest.get_item_group(itemname, "head_accessory") > 0 then
            ...
            player_acc_visuals[name].head = player_acc_visuals[name].head or {}
            if player_acc_visuals[name].head[accessorySlot] then
                player_acc_visuals[name].head[accessorySlot]:remove()
                player_acc_visuals[name].head[accessorySlot] = nil
            end
            local entity = minetest.add_entity(pos, "equippable_accessories:acc_display_ent")
            player_acc_visuals[name].head[accessorySlot] = entity
            entity:set_attach(player, "Head", headPos, {x=0, y=180, z=0})
            entity:set_animation(headAnim, 24, 0)
            local dyeStuff = ""
            ...
            if current_dye_stack:get_definition() and current_dye_stack:get_definition().accessory_dye then
                dyeStuff = current_dye_stack:get_definition().accessory_dye
            end
            entity:set_properties({textures={headTexture..dyeStuff}, mesh=headModel, visual_size=headSize, glow=headGlow})
        end
        ...
        if minetest.get_item_group(itemname, "body_accessory") > 0 then
            ...
            player_acc_visuals[name].body = player_acc_visuals[name].body or {}
            if player_acc_visuals[name].body[accessorySlot] then
                player_acc_visuals[name].body[accessorySlot]:remove()
                player_acc_visuals[name].body[accessorySlot] = nil
            end
            local entity = minetest.add_entity(pos, "equippable_accessories:acc_display_ent")
            player_acc_visuals[name].body[accessorySlot] = entity
            entity:set_attach(player, "Body", bodyPos, {x=0, y=180, z=0})
            entity:set_animation(bodyAnim, 24, 0)
            local dyeStuff = ""
            local dye_inv = minetest.get_inventory({type="detached", name="accs_dye_"..name})
            local current_dye_stack = dye_inv:get_stack("accs_dye", index)
            if current_dye_stack:get_definition() and current_dye_stack:get_definition().accessory_dye then
            ...
            entity:set_properties({textures={bodyTexture..dyeStuff}, mesh=bodyModel, visual_size=bodySize, glow=bodyGlow})
        end

        if minetest.get_item_group(itemname, "l_arm_accessory") > 0 and accessorySlot ~= "r_hand_acc" and accessorySlot ~= "r_ring_acc" then
            local l_armModel = "simple_hand.b3d"
            local l_armTexture = "simple_test_hand.png"
            local l_armPos = {x=0, y=2.0, z=0}
            local l_armAnim = {x=0, y=0}
            local l_armSize = {x=2.4, y=2.4}
            local l_armGlow = 0
            local ok, def = pcall(accessory.get_definition, accessory)
            if ok and def then
                l_armModel = def.acc_model_l_arm or l_armModel
                l_armTexture = def.acc_texture_l_arm or l_armTexture
                l_armPos = def.acc_pos_l_arm or l_armPos
                l_armAnim = def.acc_anim_l_arm or l_armAnim
                l_armSize = def.acc_size_l_arm or l_armSize
                l_armGlow = def.acc_glow_l_arm or l_armGlow
            end
            player_acc_visuals[name].l_arm = player_acc_visuals[name].l_arm or {}
            if player_acc_visuals[name].l_arm[accessorySlot] then
                player_acc_visuals[name].l_arm[accessorySlot]:remove()
                player_acc_visuals[name].l_arm[accessorySlot] = nil
            end
            local entity = minetest.add_entity(pos, "equippable_accessories:acc_display_ent")
            player_acc_visuals[name].l_arm[accessorySlot] = entity
            entity:set_attach(player, "Arm_Left", l_armPos, {x=0, y=180, z=180})
            entity:set_animation(l_armAnim, 24, 0)
            local dyeStuff = ""
            local dye_inv = minetest.get_inventory({type="detached", name="accs_dye_"..name})
            -- Convert string slot to numeric index and get the stack
			local numeric_index = table.index(player_accessory_slots, accessorySlot)
			local current_dye_stack = numeric_index and dye_inv:get_stack("accs_dye", numeric_index) or nil

            if current_dye_stack:get_definition() and current_dye_stack:get_definition().accessory_dye then
                dyeStuff = current_dye_stack:get_definition().accessory_dye
            end
            entity:set_properties({textures={l_armTexture..dyeStuff}, mesh=l_armModel, visual_size=l_armSize, glow=l_armGlow})
        end

        if minetest.get_item_group(itemname, "r_arm_accessory") > 0 and accessorySlot ~= "l_hand_acc" and accessorySlot ~= "l_ring_acc" then
            local acc_def = accessory:get_definition() or {}
            local r_armModel = acc_def.acc_model_r_arm or "simple_hand.b3d"
            local r_armTexture = acc_def.acc_texture_r_arm or "simple_test_hand.png"
            local r_armPos = acc_def.acc_pos_r_arm or {x=0, y=2.0, z=0}
            local r_armAnim = acc_def.acc_anim_r_arm or {x=0, y=0}
            local r_armSize = acc_def.acc_size_r_arm or {x=2.4, y=2.4}
            local r_armGlow = acc_def.acc_glow_r_arm or 0
            player_acc_visuals[name].r_arm = player_acc_visuals[name].r_arm or {}
            if player_acc_visuals[name].r_arm[accessorySlot] then
                player_acc_visuals[name].r_arm[accessorySlot]:remove()
                player_acc_visuals[name].r_arm[accessorySlot] = nil
            end
            local entity = minetest.add_entity(pos, "equippable_accessories:acc_display_ent")
            player_acc_visuals[name].r_arm[accessorySlot] = entity
            entity:set_attach(player, "Arm_Right", r_armPos, {x=0, y=180, z=180})
            entity:set_animation(r_armAnim, 24, 0)
            local dyeStuff = ""
            local dye_inv = minetest.get_inventory({type="detached", name="accs_dye_"..name})
            local current_dye_stack = dye_inv:get_stack("accs_dye", index)
            if current_dye_stack:get_definition() and current_dye_stack:get_definition().accessory_dye then
            ...
            entity:set_properties({textures={r_armTexture..dyeStuff}, mesh=r_armModel, visual_size=r_armSize, glow=r_armGlow})
        end

        if minetest.get_item_group(itemname, "l_leg_accessory") > 0 then
            local l_legModel = accessory:get_definition().acc_model_l_leg or "simple_hand.b3d"
            ...
            player_acc_visuals[name].l_leg = player_acc_visuals[name].l_leg or {}
            if player_acc_visuals[name].l_leg[accessorySlot] then
                player_acc_visuals[name].l_leg[accessorySlot]:remove()
                player_acc_visuals[name].l_leg[accessorySlot] = nil
            end
            local entity = minetest.add_entity(pos, "equippable_accessories:acc_display_ent")
            player_acc_visuals[name].l_leg[accessorySlot] = entity
            entity:set_attach(player, "Leg_Left", l_legPos, {x=0, y=0, z=180})
            entity:set_animation(l_legAnim, 24, 0)
            local dyeStuff = ""
            local dye_inv = minetest.get_inventory({type="detached", name="accs_dye_"..name})
            local current_dye_stack = dye_inv:get_stack("accs_dye", index)
            if current_dye_stack:get_definition() and current_dye_stack:get_definition().accessory_dye then
            ...
            entity:set_properties({textures={l_legTexture..dyeStuff}, mesh=l_legModel, visual_size=l_legSize, glow=l_legGlow})
        end

        if minetest.get_item_group(itemname, "r_leg_accessory") > 0 then
            local r_legModel = accessory:get_definition().acc_model_r_leg or "simple_hand.b3d"
            ...
            player_acc_visuals[name].r_leg = player_acc_visuals[name].r_leg or {}
            if player_acc_visuals[name].r_leg[accessorySlot] then
                player_acc_visuals[name].r_leg[accessorySlot]:remove()
                player_acc_visuals[name].r_leg[accessorySlot] = nil
            end
            local entity = minetest.add_entity(pos, "equippable_accessories:acc_display_ent")
            player_acc_visuals[name].r_leg[accessorySlot] = entity
            entity:set_attach(player, "Leg_Right", r_legPos, {x=0, y=0, z=180})
            entity:set_animation(r_legAnim, 24, 0)
            local dyeStuff = ""
            local dye_inv = minetest.get_inventory({type="detached", name="accs_dye_"..name})
            local current_dye_stack = dye_inv:get_stack("accs_dye", index)
            if current_dye_stack:get_definition() and current_dye_stack:get_definition().accessory_dye then
            ...
            entity:set_properties({textures={r_legTexture..dyeStuff}, mesh=r_legModel, visual_size=r_legSize, glow=r_legGlow})
        end
    end

    if addtype == 0 then
        if player_acc_visuals[name] then
            if player_acc_visuals[name].head[accessorySlot] then
                player_acc_visuals[name].head[accessorySlot]:remove()
                player_acc_visuals[name].head[accessorySlot] = nil
            end
...
-- Save main accessory stats in meta
...
-- Apply set bonuses if you have that function
if apply_accessory_setBonus then
    apply_accessory_setBonus(player)
end

-- Update player physics
player:set_physics_override({
    speed   = 1 + acc_walkSpeed,
    jump    = 1 + acc_jumpHeight,
    gravity = 1 + acc_gravPull,
    fly     = (acc_flight_time > 0) or minetest.check_player_privs(player:get_player_name(), {fly=true})
})

-- Set max HP
player:set_properties({hp_max = 20 + acc_hpBonus})

-- Save wings stats for later
...
playerMeta:set_int("accessory_hpBonus",acc_hpBonus)
local name = player:get_player_name()
local has_fly_priv = minetest.check_player_privs(name, {fly=true})

-- Base flight stats from wings
local flight_time = acc_flight_time
local flight_force = acc_flight_force

-- Apply wings stats even if fly priv exists
playerMeta:set_float("accessory_flightTime", flight_time)
playerMeta:set_float("accessory_flightForce", flight_force)
...
function ApplyAccessoryDmgBonus()
    for _, ent in pairs(minetest.registered_entities) do
        if type(ent.on_punch) == "function" then
            ...
            ent.on_punch = function(self, hitter, time_from_last_punch, tool_capabilities, direction, pos)
                ...
                and tool_capabilities.damage_groups
                and tool_capabilities.damage_groups.fleshy then

                    local atk_bonus = hitter:get_meta():get_int("accessory_offense") or 0

                    tool_capabilities.damage_groups.fleshy =
                        tool_capabilities.damage_groups.fleshy * (1 + atk_bonus / 100)
                end
...
-- [All your armor registrations above]
-- Wooden, Copper, Tin, Bronze, Steel, Golden, Mithril, Silver, Cactus, Dungeon, Molten

-- ===== Disable all armor except Dungeon Warrior and Molten =====
local remove_armor = {
    "equippable_accessories:wooden_helmet",
    "equippable_accessories:wooden_armor",
    "equippable_accessories:wooden_leggings",
    "equippable_accessories:copper_helmet",
    "equippable_accessories:copper_chainmail",
    "equippable_accessories:copper_leggings",
    "equippable_accessories:tin_helmet",
    "equippable_accessories:tin_chainmail",
    "equippable_accessories:tin_leggings",
    "equippable_accessories:bronze_helmet",
    "equippable_accessories:bronze_scalemail",
    "equippable_accessories:bronze_leggings",
    "equippable_accessories:steel_helmet",
    "equippable_accessories:steel_scalemail",
    "equippable_accessories:steel_leggings",
    "equippable_accessories:golden_helmet",
    "equippable_accessories:golden_platemail",
    "equippable_accessories:golden_leggings",
    "equippable_accessories:mithril_helmet",
    "equippable_accessories:mithril_platemail",
    "equippable_accessories:mithril_leggings",
    "equippable_accessories:silver_helmet",
    "equippable_accessories:silver_platemail",
    "equippable_accessories:silver_leggings",
    "equippable_accessories:cactus_helmet",
    "equippable_accessories:cactus_armor",
    "equippable_accessories:cactus_leggings"
}

for _, name in ipairs(remove_armor) do
