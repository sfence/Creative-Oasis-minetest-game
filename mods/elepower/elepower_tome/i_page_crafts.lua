...
function eletome.craft_page(craft_type,page_num)
	-- remove "action:" from string
	craft_type = string.match(craft_type, ":(.*)")

	-- Convert craft_type back to craft
	local raw_all_crafts = table.copy(elepm.craft.types)

	local craft_click = {}
	local craft_description = ""
	for craft_name,def in pairs(raw_all_crafts) do
		if craft_name == craft_type then
			craft_description = def.description
	...
	--------------------------
	local overview_text   = (craft_click[craft_type].overview or ""):gsub(";", "\\;")
	local heading_lp      = "hypertext[0.5,0.7;8.5,1.1;craft_lp_h;"..sty_h0s..craft_description..sty_h0e.."]"
	local head_sub_lp_ov  = "hypertext[0.5,1.4;8.5,1.0;craft_lp_sh_ov;"..sty_h1s..S("Overview")..sty_h1e.."]"
	local ov_lp_txt       = "hypertext[0.75,2.1;8.0,5.5;craft_lp_ov_text;"..sty_h3s..overview_text..sty_h3e.."]"

	-- calculate approximatly how many lines "overview" takes
	local length = string.len(overview_text)
	...
	---------------------------
	local head_sub_rp_rec      = "hypertext[9.5,0.7;8.5,1.0;craft_lp_h;"..sty_h1s..S("Recipes")..sty_h1e.."]"
	...
	local recipe_rp = eletome.p_nav_bwd_fwd(craft_type,recipe_list_rdy,max_recipe_pp,pg_num,"craft_bwd_fwd")
