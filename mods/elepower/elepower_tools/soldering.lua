...
local function upgrade_formspec(upgrades, desc)
    ...
    return start .. efs.label(bx, by, S("Modifying @1", desc)) .. fspec ..
...
local function set_component_list(pos, list)
    ...
    local len = 0
    for _, v in pairs(list) do if v ~= nil then len = len + 1 end end
...
local function machine_modify(pos, node, user)
    ...
    if not nodedef.ele_upgrades then
        return minetest.chat_send_player(user:get_player_name(),
                                         S("This machine cannot be modified."))
    end
    ...
    local inv = minetest.create_detached_inventory("soldering", {
        allow_move = function(inv, from_list, from_index, to_list, to_index,
                              count, player) return 0 end,

        allow_put = function(inv, listname, index, stack, player)
            if minetest.get_item_group(stack:get_name(), "ele_upgrade_component") ==
                0 or minetest.get_item_group(stack:get_name(), listname) < 2 then
                return 0
            end
    ...
    for k in pairs(nodedef.ele_upgrades) do
        ...
        if comps[k] then inv:set_stack(k, 1, ItemStack(comps[k])) end
    end

    -- Open the formspec
    minetest.show_formspec(user:get_player_name(),
                           "elepower_tools:soldering_iron", upgrade_formspec(
                               nodedef.ele_upgrades, nodedef.description))
end

minetest.register_on_player_receive_fields(
    function(player, formname, fields)
        if formname ~= "elepower_tools:soldering_iron" or not player then
            return false
        end
...
minetest.register_on_leaveplayer(function(player)
    ...
    local meta = minetest.get_meta(pos)
    if meta:get_int("drop_lock") == 1 then meta:set_int("drop_lock", 0) end
    umeta:set_string("soldering", "")
end)

ele.register_tool("elepower_tools:soldering_iron", {
    description = S("Soldering Iron") .. "\n" ..
        S("Used to replace components in machines"),
    ...
    on_use = function(itemstack, user, pointed_thing)
        ...
        if pointed_thing and pointed_thing.type == "node" then
            ...
            if not node or node.name == "air" or
                minetest.is_protected(pos, user:get_player_name()) then
                return itemstack
            end
