...
-- tubelib2.get_node_lvm is used when available; otherwise fall back to minetest.get_node
local get_node_lvm = (tubelib2 and tubelib2.get_node_lvm) or minetest.get_node
...
-- Patched: safe, backward-compatible swap_node
function signs_bot.lib.swap_node(pos, node_name)
	-- node_name may be a table or string; mimic minetest.swap_node behaviour
	minetest.swap_node(pos, node_name)
	local mod = node_name.name:match("(.*):")
	if mod == "farming" then
		-- If the farming mod still provides the old helper, use it (backwards compatibility).
		if farming and type(farming.start_seed_timer) == "function" then
			-- pcall to avoid crashing if the farming function errors
			pcall(farming.start_seed_timer, pos)
		else
			-- Use Minetest node timers (modern API).
			-- Start with a small delay (1 second) so any after_place_node handlers run first.
