...
-- Patch: allow deployers to throw items if they cannot place or use them
pipeworks.activate_deployer = function(pos, node)
    ...
    local pointed_thing = {
        type = "node",
        under = target_pos,
        above = vector.add(target_pos, dir),
    }
    ...
    if def.on_place then
        stack = def.on_place(stack, fake_player, pointed_thing) or stack
    end

    -- try using
    if stack:get_count() > 0 and def.on_use then
        stack = def.on_use(stack, fake_player, pointed_thing) or stack
    end

    -- throw if nothing happened
    if stack:get_count() == original:get_count() then
        local obj = minetest.add_item(vector.add(pos, dir), stack:take_item(1))
        if obj then
