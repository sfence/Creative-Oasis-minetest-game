...
-- Clamp the radius when receiving fields
local function object_detector_on_receive_fields(pos, _, fields, player)
	if not fields.scanname or not fields.digiline_channel or
	...
	if r then
		clamp_and_save(meta, "radius", r, 1, 20)  -- clamp safely, no formspec resync
	end
end
...
local object_detector_digiline = {
	effector = {
		action = function(pos, node, channel, msg)
			...
			if channel ~= active_channel then return end
			...
			elseif type(msg) == "table" then
				if msg.radius then
					...
					if r then
						clamp_and_save(meta, "radius", r, 1, 20)
					end
				end
