...
replacer.inspect = function( itemstack, user, pointed_thing, mode, show_receipe )
    ...
    if not ref then
        text = text.."a broken object. No further info."
                ...
                if sdata then
                    ...
                    if sdata and sdata.itemstring then
                        ...
                        if show_receipe then
                            replacer.inspect_show_crafting(name, sdata.itemstring, {pos=ref:get_pos(), luaob=luaob})
                        end
                    end
                    if sdata and sdata.age then
                        text = text..', dropped '..tostring(math.floor(sdata.age/60))..' minutes ago'
                    end
                    if sdata and sdata.owner then
                        text = text..' (owned by '..minetest.formspec_escape(sdata.owner)..")"
                    end
                end
            end
        else
            text = text.."object \""..tostring(ref:get_entity_name() or "unknown").."\""
        end
    end
...
elseif pointed_thing.type ~= 'node' then
    minetest.chat_send_player(name, 'Sorry. This is an unknown something of type \"'..tostring(pointed_thing.type)..'\". No information available.')
    return nil
end
