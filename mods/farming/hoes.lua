...
core.register_tool("farming:scythe_mithril", {
    ...
    if not def or not def.groups or not def.groups.plant then
        return
    end

    -- get drops BEFORE removing/replanting
    ...
    -- determine first growth stage node
    local repl_name = mname .. ":" .. pname .. "1"

    -- replant or remove if first stage doesn't exist
    if core.registered_nodes[repl_name] then
        local p2 = core.registered_nodes[repl_name].place_param2 or 0
        core.set_node(pos, {name = repl_name, param2 = p2})
    else
        core.remove_node(pos)
    end

    -- drop harvested items except those in scythe_not_drops
    for _, dropped_item in pairs(drops) do
        ...
        if not skip then
            ...
            if obj then
                obj:set_velocity({x = math.random() - 0.5, y = 3, z = math.random() - 0.5})
            end
        end
    end
